...
const AbsenceContainer: React.FC = () => {
    ...
    const fetchPeople = async () => {
      ...
      try {
        ...
        if (personType === "doctor") {
          ...
          const transformedEmployees: Employee[] = rawEmployees.map(
            (employee) => ({
              id: employee.id,
              name: employee.name,
      ...
      if (response?.status === "success") {
        ...
        if (!attendanceId) {
          sweetAlertInstance.fire({
            title: "خطأ",
            text: `لا يمكن تسجيل غياب لهذا اليوم. تأكد من اختيار يوم حضور صحيح.`,
            icon: "error",
  ...
  const handleUpdateAbsence = async (
    ...
    try {
      ...
      const attendanceId = existingAttendance?.id;
      const response = await updateAbsenceStatus(
        date,
        personInfo,
        personType,
        attendanceId
  ...
  const handleSwapDaySubmit = async (data: SwapDayFormData) => {
    ...
    try {
      ...
      if (response.status === "success") {
        ...
        const switchedDate =
          response.data.response.data.existing_switch.switched_date;

        console.log(errorMessage);
        if (
          errorMessage === "This day has already been submitted for switching"
        ) {
          await sweetAlertInstance.fire({
            icon: "error",
