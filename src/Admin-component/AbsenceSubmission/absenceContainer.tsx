...
const AbsenceContainer: React.FC = () => {
  const accessToken = sessionStorage.getItem("accessToken");
  const [selectedPersonAttendance, setSelectedPersonAttendance] = useState<
    AttendanceData[]
  >([]);
  ...
  const submitAbsence = async (date: Date, personInfo: PersonOption) => {
    const dayInfo = formatDateWithArabicDay(date);
    const dayID = getChosenDayID(dayInfo, personInfo);

    const result = await sweetAlertInstance.fire({
      title: "هل انت متاكد ؟",
      text: `سيتم تسجيل غياب ${
    ...
    if (result.isConfirmed) {
      try {
        ...
        if (response.data.message.includes("attendance already taken")) {
          ...
          // Update attendance data instead of reloading
          await getAttendanceByPersonID(personInfo.value, personType);
        }
      } catch (error) {
        console.log(error);
        sweetAlertInstance.fire({
        ...
        .then((response) => {
          if (response.status === 200) {
            sweetAlertInstance.fire({
              ...
              text: `تم تعديل حالة غياب ${
                personType === "doctor" ? "الطبيب" : "الموظف"
              } "${personInfo.label}" يوم "${dayInfo.arabicDayName}" الموافق "${
                dayInfo.formattedDate
              }" `,
            });
            getAttendanceByPersonID(personInfo.value, personType);
          }
        })
        .catch((error) => {
          console.log(error);
          if (
            ...
            sweetAlertInstance.fire({
              icon: "error",
              title: "تم تعديل حاله الغياب مسبقاً",
              text: `تم تعديل حالة هذا ${
                personType === "doctor" ? "الطبيب" : "الموظف"
              } كغائب لهذا اليوم بالفعل.`,
            });
          } else {
            sweetAlertInstance.fire({
              icon: "error",
  ...
  const handleSwapDaySubmit = async (data: SwapDayFormData) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        // Refresh the attendance data
        await getAttendanceByPersonID(selectedPerson.value, personType);
      }
    } catch (error) {
      console.error(error);
      sweetAlertInstance.fire({
